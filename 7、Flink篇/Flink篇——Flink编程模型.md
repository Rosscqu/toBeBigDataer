...
1）利用StreamExecutionEnvironment的静态方法getExecutionEnvironment()创建Flink运行的上下文环境；

2）初始化数据，监听127.0.0.1:9000获取数据；

3）执行业务处理逻辑：对原始数据按行处理，使用flatMap将数据切分并生成(word,1)的键值对，并使用keyBy按word进行分组，因为是流式数据处理，所以选择窗口大小为5s的滑动窗口进行求和；
...
2）创建Source算子，这里是按行读取本地的文件；

3）处理业务逻辑：对于每一行数据进行切分成（word，1）的键值对，然后对键进行分组并求和；

4）将统计结果输出到控制台；

5）最后使用env提交整个任务即可执行任务。计算结束后任务也就结束了，打印在控制的结果如下：
...
- 初始化Stream执行环境
- 配置参数：执行环境参数、业务参数
- 读取外部数据：kafka、HDFS、HBase、数据库等；
- 数据处理流程
- 将处理结果写入外部
- 触发执行：StreamExecutionEnvironment#execute

调用完该API后，会触发一系列生成StreamGraph、JobGraph、ExecutionGraph和任务分发执行的过程。